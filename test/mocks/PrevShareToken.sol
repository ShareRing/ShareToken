...
/**
 * The pragma solidity is udpated to 0.8.20 to match the version of the Foundry Test contract
 * TODO: list minor changes
 */
...
contract ERC20TokenExtended is IERC20 {
    ...
    modifier migrateAllowance(address _owner, address _spender) {
        if (!migratedAllowances[_owner][_spender]) {
            ...
            allowed[_owner][_spender] = allowed[_owner][_spender].add(prevContract.allowance(_owner, _spender));
        }
        _;
    }
...
contract ShareTokenExtended is ERC20TokenExtended, WhiteListManager {
    ...
    constructor(address _prevContract) ERC20TokenExtended(_prevContract) {
